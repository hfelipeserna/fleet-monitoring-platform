...
- **Main Flow**:
  ...
  5. Tool devuelve `[{plate, zone_name, stopped_since, duration_min, lat, lon}]` minimizado (máx 20 filas, 3 dec, sin PII)
   ...
   7. `cmd/api` filtra y reenvía a SPA; SPA renderiza **solo** `reply` en markdown y resalta marker en mapa; las `citations` se mantienen en el payload para trazabilidad pero **no se renderizan como línea `tool count` (ej. `listPlates 7`)** ni como badge de placa en el widget
- **Alternative Flows**:
...
| BR-004 | Minimización: máx 20 vehículos por respuesta, campos mínimos, precisión lat/lon máx 6 dec (default 3), preferencia por agregados sobre listas crudas; sin PII de conductor; sin `client_event_id` (ADR-0003 cond. 4, GDPR art. 5). **UI limpia:** no renderizar `citations` como `listPlates 7`/`findVehicles... 2` ni badge `ABC123` en el input; solo `reply` markdown + highlight en mapa | UC-001/002, FR-002/005 |
...
AC-002 (UC-001, FR-003, BR-002/009):
  Given plate LMN456 existe con lat 4.71 lon -74.07 speed 45 received_at 2026-08-25T09:41:10Z y plate GTP98 es inválida
  When POST /api/chat {message:"¿dónde está LMN456 o estado de GTP980 o posición de LMN456?"} -> tool getVehicleStatus {plate:"LMN456"/"GTP980"}
  Then primer request 200 con reply que contiene lat, lon, received_at y status de la placa (ej. "Vehículo LMN456 estado en movimiento, última posición lat 4.710000 lon -74.070000 recibida 2026-08-25T09:41:10Z velocidad 45 km/h") filtrado 6 dec; segundo con GTP98 400 {error:"validation", details:["plate ^[A-Z]{3}[0-9]{3}$"]} sin llamar a Gemini para el inválido si el BFF lo valida, o tool retorna 400 interno traducido. Cuando se pide "posición de LMN456" debe incluir coordenadas + hora, no solo "en movimiento"
