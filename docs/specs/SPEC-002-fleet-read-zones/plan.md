...
| **3** Zones CRUD | `fleet/domain/zone_test.go` reuse + `adapters/http/zone_test.go` + `adapters/pg/zone_test.go` | `POST`: `Polygon válido 201 + ST_IsValid true + ST_Area>0`, `no cerrado /3pts/área0/101+1 -> 400 sin INSERT`, `name blank/101 ->400`, `name duplicado case-insensitive ->409 (UNIQUE lower(name) 0003)`, `PUT id random UUID ->404`, `PUT geo inválido ->400`, `PUT name duplicado ->409`, `DELETE ->204, 404 segunda vez` | AC-003/004, BR-002/005 | unit 400/409 paths + integration pg |
...
**Implementation**: `ST_GeomFromGeoJSON($1)` con `ST_IsValid` check, `ST_SRID 4326`, `GIST` overlay, `UNIQUE lower(name)` (0003) → `409`, `429 10/min IP` bucket zones, `400` vs `404` vs `409` mapping, GeoJSON `FeatureCollection` marshal `ST_AsGeoJSON(geom)::json`
