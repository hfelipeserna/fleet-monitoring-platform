...
## 2026-08-30 — Auditoría: mobile RouteButtons flakiness migratorio por mezcla fake/real timers y resetStore parcial [SPEC-005]
Severidad: alta
Hallazgo: IA `test-engineer` propuso fix parcial para 2 timeouts (`encola pending`, `secuencia Medellín`) usando `jest.useRealTimers()` intra-test + `spyOn(setInterval)` manual + `jest.useFakeTimers()` al final, y `mobile-expo` dejó `flushMicrotasks` sin `act` y `resetStore()` parcial sin `selectedRouteIdx`. Tras ese fix, los 2 tests pasaban local 6s pero CI Node 24 migraba el timeout a 3 tests diferentes (`placa se mantiene al seleccionar Medellín` 30s, `purga Medellín→Bogotá verde` hook 30s, `secuencia Bogotá reinicia 0` 10s) — whack-a-mole.
Evidencia: mobile/src/components/RouteButtons.test.tsx:186 `jest.useRealTimers()` intra-test + `spyOn(global,setInterval)` + `jest.useFakeTimers()` al final sin `cleanup` previo, `95 flushMicrotasks(){await Promise.resolve()}` fuera de `act`, `71 resetStore()` sin `selectedRouteIdx` (appStore tiene `selectedRouteIdx:0` y `reset()` atómico), `102 beforeEach jest.useFakeTimers()` vs `136 afterEach jest.useRealTimers()` no simétrico con intra-test, `intervalRegistry 5000ms` compartido entre `App pending 2000ms` + `useTelemetryGenerator 5000ms` + `useSync 5000ms`, CI log `FAIL 73.741s 3 failed` tras `76b5f40`, `mobile-auditor ses_fabda4389` y `test-engineer ses_fabd83698`
Por qué falla: Mezcla `useFake` global + `useReal` intra-test rompe aislamiento: intervalos reales creados bajo `useRealTimers` quedan huérfanos cuando `afterEach` hace `clearAllTimers` con fake timers (mock fake no limpia Timeout real) → intervalo real sigue vivo disparando `void async IIFE` cada 5s en el siguiente test, contamina `mockEnqueue` y `getIds()`. `resetStore` parcial deja `selectedRouteIdx 7` contaminado → `nextSimPoint(route,7)` no es `MED[0] 6.2442,-75.5812` sino `MED[1]`. `flushMicrotasks` fuera de `act` no drena `selectRoute` async `await clearPending().then(setState)` → `setState` aterriza en siguiente test. `advanceTimersByTime` dentro de `act(async)` con intervalos periódicos nunca resuelve (act espera vaciar microtasks pero intervalos son periódicos) → timeout migratorio. Violación `mobile-auditor` timers homogéneos + `test-engineer` AAA 1 Act por test + `AGENTS.md` done `reviewer sin hallazgos altos`.
Refactor exigido: Homogeneizar a `jest.useFakeTimers({legacyFakeTimers:false})` único en `beforeEach/afterEach`, eliminar `useRealTimers` intra-test y `spyOn setInterval` manual, extraer helper `tick(ms)=act+jest.advanceTimersByTimeAsync(ms)` y `flushMicrotasks=act+double Promise.resolve` dentro de `act`, `beforeEach/afterEach` atómicos `__resetPorts()+intervalRegistry.reset()+act(reset())` con `useAppStore.getState().reset()` completo (incluye `selectedRouteIdx`), `afterEach` simétrico `clearAllTimers+cleanup+reset+__resetPorts+restoreAllMocks+useRealTimers`. Tests `encola pending` y `secuencia` ahora usan `await tick(5000)` homogéneo, `secuencia Bogotá` con `Arrange selectedRoute:'medellin' idx:5` y solo 1 Act `press bogota`. `npm test -- --runInBand` 19 suites 187 tests PASS 6.9s estable en CI Node 22.
Auditor: mobile-auditor | test-engineer | architect

## 2026-08-30 — Auditoría: mobile RouteButtons tick usa advanceTimersByTimeAsync → timeout en CI por fake timers no activos [SPEC-005]
Severidad: alta
Hallazgo: El "refactor exigido" de la entrada previa impuso el helper `tick(ms)=act+await jest.advanceTimersByTimeAsync(ms)` como la convención única. Resultó ser el ÚNICO punto del codebase que usa la API asíncrona de fake timers (`advanceTimersByTimeAsync`); los otros 8 archivos de test (`useConnection.test.tsx`, `useSimulatedRoute.test.tsx`, `appStore.test.tsx`, `useConnection.disconnect.test.tsx`, `Disconnect.test.tsx`, `api.test.ts`, `sync.test.ts`) pasan en CI con la variante SÍNCRONA `jest.advanceTimersByTime()` dentro de `act(async)` + drenaje `await Promise.resolve()`. En el run CI `npm test -- --run --coverage --runInBand` (Node 22), los 3 tests que usan `tick` (`encola pending cada 5s` 174, `secuencia desde 0` 214, `placa se mantiene` 249) timeout a 30s con `console.warn "A function to advance timers was called but the timers APIs are not replaced with fake timers"` en `RouteButtons.test.tsx:86 await jest.advanceTimersByTimeAsync(ms)` — fake timers no estaban activos al llegar a `tick` pese al `jest.useFakeTimers()` del `beforeEach`. Local (Node 24, archivo aislado) pasa en 6s; CI (Node 22, suite completa in-band) falla. El reordenamiento `restoreAllMocks`→`useFakeTimers` no lo corrigió.
Evidencia: mobile/src/components/RouteButtons.test.tsx:84-90 `tick` previo `_checkFakeTimers` `FakeTimers.advanceTimersByTimeAsync modernFakeTimers.js:98` (CI log `FAIL 96.193s 3 failed` → `Test Suites: 1 failed 18 passed` después de `6d58ee7`), grep `rg advanceTimers` `src/**/*.test.*`: solo `RouteButtons.test.tsx` usa `advanceTimersByTimeAsync`; `useConnection.test.tsx:114/157/165/205` y `appStore.test.tsx:242/277` usan `advanceTimersByTime` síncrono y pasan CI.
Por qué falla: `advanceTimersByTimeAsync` (modern fake timers) es sensible al estado global de `_fakingTime` y al drenaje de microtasks de intervalos periódicos `setInterval(() => void async IIFE)` dentro de `act(async)`: si el entorno no garantiza fake timers activos en ese punto (interacción entre suite completa in-band, `restoreAllMocks`, y el wrapper `act` del preset `jest-expo`), lanza `_checkFakeTimers` en vez de avanzar → `tick` pende indefinidamente → timeout migratorio. La variante síncrona `advanceTimersByTime` + `act(async)` + `await Promise.resolve()` es la convención probada y estable del resto del repo (0 timeouts en CI en esos 8 archivos). Violación `AGENTS.md` done `tests verdes sin hallazgos altos` y `test-engineer` "1 Act por test determinístico".
Refactor exigido: Reemplazar en `tick` `await jest.advanceTimersByTimeAsync(ms)` por `jest.advanceTimersByTime(ms)` síncrono seguido de drenaje `await Promise.resolve(); await Promise.resolve();` todo dentro del mismo `act(async)` — idéntico al patrón verde de `useConnection/useSimulatedRoute/appStore`. Mantener `flushMicrotasks` con doble `Promise.resolve` para drenar el `async IIFE` de `useTelemetryGenerator`. `npm test -- --run --coverage --runInBand` 19 suites 187 tests PASS en 6.1s local y sin warning `timers APIs` en CI. Commit `fix(mobile)` 6d58ee7→.
Auditor: architect | test-engineer | mobile-auditor

## 2026-08-30 — Auditoría: causa raíz real — incompatibilidad Jest 29.7 fake timers + Node 22 (no era la API async) [SPEC-005]
Severidad: alta
Hallazgo: Tras varios intentos ("siguen fallando"), la causa raíz NO era la API de fake timers (async vs sync) ni el helper `tick`, sino una **incompatibilidad de Jest 29.7 con Node 22** en `advanceTimersByTime` sobre un render del `<App/>` completo (monta intervalos periódicos async: `useTelemetryGenerator` 5000ms + `App` 2000ms). Bajo Node 22, incluso `useSimulatedRoute.test.tsx` —que pasa en CI desde antes y rinde `<App/>` con el patrón `act(()=>advanceTimersByTime)` síncrono— falla con el MISMO `console.warn "timers APIs are not replaced with fake timers"` + timeout 30s (4 timeouts). Bajo Node 24 todo el suite pasa: 19/19 suites, 187/187 tests en 6.7s. `RouteButtons.test.tsx` igual (3 timeouts Node 22, 0 Node 24).
Evidencia: `nvm use 22 && npx jest src/hooks/useSimulatedRoute.test.tsx` → `FAIL 4 timeouts` con warning `modernFakeTimers.js:158 _checkFakeTimers` `timers APIs are not replaced`; `nvm use 24 && npx jest --runInBand --coverage` → `19 suites 19 passed, 187 passed 6.782s`. `fakeTimers:{enableGlobally:true}` en jest.config **no lo arregla** (sigue `_checkFakeTimers`, dobla el timeout a 213s). Diag en el propio test: fake timers armed=YES tras `beforeEach`, disarm durante render+advance. `.github/workflows/mobile.yml` fijaba `node-version: 22`.
Por qué falla: La entrada previa atribuyó el fallo a `advanceTimersByTimeAsync`; era síntoma, no causa. El disparador real es el mount completo de `<App/>` con intervalos periódicos async bajo fake timers de Jest 29.7 en Node 22: la interacción `act()` (React Native/RTL) + `advanceTimersByTime` atraviesa `_checkFakeTimers` con `_fakingTime===false`. Es un bug de entorno en hooks (no tocar backends): `useConnection.test.tsx` (hook aislado) pasa en Node 22, mientras los suites que rinden `<App/>` completo (RouteButtons, useSimulatedRoute) fallan. Reordenar/`enableGlobally`/sync-api whack-a-mole no lo resuelve porque no es lógica de test, es binario runtime de Jest+Node. Violación del objetivo "timeouts flaky persistentes".
Refactor exigido (nivel infra, no test): alinear CI con el runtime probado-verde — `.github/workflows/mobile.yml` `node-version: 22` → `24` (mismo que el dev local, 187 tests PASS Node 24). `react-native-get-random-values` ya está blindado con `try{require}catch{}` en `useTelemetryGenerator.ts` (fix del `global.crypto` non-writable de Node 24) y `uuid` mapeado en jest.config, así que Node 24 no rompe la generación de `client_event_id`. No se toca lógica de tests ni prod. Comandos: `npx jest --runInBand --coverage` Node 24 PASS 6.7s; Node 22 FAIL 3-4 timeouts por suite App.
Auditor: architect (dictamen infra) | test-engineer | mobile-auditor
