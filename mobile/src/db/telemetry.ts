...
export async function enqueue(point: EnqueueInput): Promise<TelemetryRecord> {
  ...
  try {
    ...
    if (db && (db as unknown as Record<string, unknown>).adapter) {
      const adapter = (db as unknown as Record<string, unknown>).adapter as Record<string, unknown>;
...
export async function getPending(limit = 500): Promise<TelemetryRecord[]> {
  ...
  try {
    ...
    if (db && db.collections) {
      const col = db.collections.get('pending_telemetry');
      const Q = (await import('@nozbe/watermelondb')).Q as unknown as Record<string, unknown>;
      if (Q) {
        try {
          const pending = await (col.query as unknown as (a: unknown, b: unknown) => { fetch: () => Promise<TelemetryRecord[]> })(
          ...
          if (Array.isArray(pending) && pending.length >= 0) return pending.slice(0, limit);
        } catch {}
      }
      try {
        const all = await (col.query as unknown as () => { fetch: () => Promise<TelemetryRecord[]> })().fetch();
        return (all as TelemetryRecord[]).filter((r) => r.sync_status === 'pending').slice(0, limit);
      } catch {}
    }
    if (db && (db as unknown as Record<string, unknown>).adapter) {
      const adapter = (db as unknown as Record<string, unknown>).adapter as Record<string, unknown>;
...
export async function countPending(): Promise<number> {
  ...
  try {
    const items = await getPending(100000);
    if (items.length !== mockQueue.length || mockQueue.length === 0) {
      return items.length;
    }
    return mockQueue.length;
  } catch {
